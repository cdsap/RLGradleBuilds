    ...
    if (build_time !== undefined && gradle_gc_time !== undefined && kotlin_gc_time !== undefined) {
      try {
        ...
        if (feedbackResponse.ok) {
          ...
          // Check if experiment should continue with next action
          try {
            const experimentDoc = await db.collection('experiments').doc(experiment_id).get();
            if (experimentDoc.exists) {
              const experimentData = experimentDoc.data();
              const repository = experimentData.repository;
              const currentIteration = experimentData.current_iteration || experimentData.iteration || 1;
              const maxIterations = 10; // Maximum number of actions per experiment
              
              logger.info('Checking iteration for continuous learning', { experiment_id, currentIteration, maxIterations });
              
              // Check if we should continue with more actions
              if (currentIteration < maxIterations) {
                logger.info('Scheduling next action for same experiment', { experiment_id, repository, currentIteration });
                
                // Update experiment with next iteration
                ...
                if (actionResponse.ok) {
                  ...
                  logger.info('Next RL action generated for same experiment', { experiment_id, rlAction, iteration: currentIteration + 1 });
                  ...
                  if (githubToken) {
                    ...
                    if (workflowResponse.ok) {
                      logger.info('Next action triggered successfully for same experiment', { experiment_id, repository, iteration: currentIteration + 1 });
                    } else {
                      const errorText = await workflowResponse.text();
                ...
                // Experiment completed - mark as finished
                logger.info('Experiment completed - maximum iterations reached', { experiment_id, repository, totalIterations: currentIteration });
                await db.collection('experiments').doc(experiment_id).update({
                  status: 'completed',
                  updated_at: admin.firestore.FieldValue.serverTimestamp()
                });
